...
const variants = cva(
  "flex flex-col select-none items-center justify-center bg-background border data-[suit=s]:text-suit-spade data-[suit=h]:text-suit-heart data-[suit=d]:text-suit-diamond data-[suit=c]:text-suit-club data-[suit=s]:fill-suit-spade data-[suit=h]:fill-suit-heart data-[suit=d]:fill-suit-diamond data-[suit=c]:fill-suit-club",
  {
    variants: {
      ...
      size: {
        sm: "h-11 w-8 min-w-8 p-px rounded border [&_span]:text-base/[1] [&>div>svg]:size-5 [&>svg]:size-6",
        md: "h-20 w-15 min-w-15 p-1 rounded-md border-2 [&_span]:text-3xl [&>div>svg]:size-8 [&>svg]:size-[48px]",
        lg: "h-36 w-28 min-w-28 p-1 rounded-md border-2 relative [&_span]:text-4xl [&>div>svg]:size-8 [&>svg]:size-22 [&_div]:absolute [&_div]:right-1 [&_div]:bottom-1",
      },
    },
