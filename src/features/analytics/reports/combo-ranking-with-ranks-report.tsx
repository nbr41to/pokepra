...
export const ComboRankingWithRanksReport = ({
  ...
  const rankingOrder = new Map(
    ranking?.map((entry, index) => [entry.hand, index]) ?? [],
  );
  const sortedData = [...result.data].sort((a, b) => {
    if (!ranking) return 0;
    return (
      (rankingOrder.get(a.hand) ?? Number.MAX_SAFE_INTEGER) -
      (rankingOrder.get(b.hand) ?? Number.MAX_SAFE_INTEGER)
    );
  });

  return (
    <div className={cn("divide-y", className)}>
      {sortedData.map(({ hand, win, tie, count, results }, index) => (
        <div
          key={hand}
          id={hand}
          className={cn(
