...
use rs_poker::core::{Card as RsCard, Hand as RsHand, Rank as RsRank, Suit as RsSuit, Value as RsValue};
...
fn rank_index(rank: &RsRank) -> usize {
  match rank {
    RsRank::HighCard(_) => 0,
    RsRank::OnePair(_) => 1,
    RsRank::TwoPair(_) => 2,
    RsRank::ThreeOfAKind(_) => 3,
    RsRank::Straight(_) => 4,
    RsRank::Flush(_) => 5,
    RsRank::FullHouse(_) => 6,
    RsRank::FourOfAKind(_) => 7,
    RsRank::StraightFlush(_) => 8,
  }
}
